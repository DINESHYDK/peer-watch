...
export const TribunalModal: React.FC<TribunalModalProps> = ({ members, currentUser, groupId }) => {
  ...
  return <TribunalContent target={target} currentUser={currentUser} groupId={groupId} date={tribunalTarget.date} onClose={closeTribunal} />
}

interface TribunalContentProps {
  target: MemberWithTasks; currentUser: UserRow
  groupId: string; date: string; onClose: () => void
}

const TribunalContent: React.FC<TribunalContentProps> = ({ target, currentUser, groupId, date, onClose }) => {
    ...
    >
      ...
      >
        ...
        <div className="bg-violet-gradient px-6 pt-6 pb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="mt-2"><StatusBadge tag={(target.status_tag as StatusTag) ?? 'New'} /></div>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
          <div>
            <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-3">Today's Performance</p>
            <ScoreSummary member={target} summary={summary ?? null} />
          </div>
          <hr className="border-bg-dark" />
          <div>
            <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-3">Your Verdict</p>
            <StarRatingInput raterId={currentUser.id} rateeId={target.id} groupId={groupId} date={date} existingRating={existingRating} onSuccess={onClose} />
          </div>
        </div>
        ...
        <div className="px-6 py-3 bg-bg border-t border-bg-dark flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-text-muted font-medium">{target.current_streak} day streak</span>
          </div>
          <div className="flex items-center gap-1">
            <Zap size={12} className="text-accent-violet" />
            <span className="text-xs font-bold text-accent-violet">{Math.round(target.global_score).toLocaleString()} pts</span>
          </div>
        </div>
      </div>
    </div>
